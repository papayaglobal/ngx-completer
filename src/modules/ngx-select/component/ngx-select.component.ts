...
} from "./ngx-select-option/ngx-select-option.component";
...
export class NgxSelectComponent
  ...
  @ContentChildren(NgxSelectOptionComponent, { descendants: true })
  public options: QueryList<NgxSelectOptionComponent>;
  ...
  @Output() public readonly valueChange: EventEmitter<any> = new EventEmitter<
    any
  >();
  @Output() public readonly selectionChange: EventEmitter<
    NgxSelectModel
  > = new EventEmitter<NgxSelectModel>();
  ...
  > = defer(() => {
    if (this.options) {
      ...
      ) as Observable<NgxOptionSelectionChange>;
    }
  ...
  public ngOnInit() {
    this._selectionModel = new SelectionModel<NgxSelectOptionComponent>(
      false,
      undefined,
      false
    );
  ...
  private initKeyManager() {
    this._keyManager = new ActiveDescendantKeyManager<NgxSelectOptionComponent>(
      this.options
    )
      .withTypeAhead()
  ...
  private handleClosedKeydown(event: KeyboardEvent): void {
    ...
    const isArrowKey =
      ...
      keyCode === RIGHT_ARROW;
    const isOpenKey =
      keyCode === ENTER || keyCode === SPACE || (event.altKey && isArrowKey);
  ...
  private scrollActiveOptionIntoView(): void {
    ...
    const scrollOffset =
      activeOptionIndex * itemHeight +
      parseNumber(getComputedStyle(selectContentEl).paddingTop);

    if (scrollOffset < panelTop) {
      selectPanelEl.scrollTop = scrollOffset;
    } else if (scrollOffset + itemHeight > panelTop + panelHeight) {
      selectPanelEl.scrollTop = Math.max(
        0,
        scrollOffset - panelHeight + itemHeight
      );
    }
  }

  private scrollToSelectedElement() {
    const activeSelectEl = this.options.toArray()[
    ...
    if (activeSelectEl) {
      ...
      selectPanelEl.scrollTop =
        activeSelectEl.element.nativeElement.offsetTop -
        selectPanelEl.offsetHeight / divider;
    }
  }
  ...
  private getOptionIndex(option: NgxSelectOptionComponent): number | undefined {
    return this.options.reduce(
      (
        ...
        current: NgxSelectOptionComponent,
        index: number
      ) => {
  ...
  private getOptionHeight(): number {
    ...
    const panelContentPaddingTop = parseNumber(
      getComputedStyle(selectContentEl).paddingTop
    );
    const panelContentPaddingBottom = parseNumber(
      getComputedStyle(selectContentEl).paddingBottom
    );
    const panelContentHeight =
      selectContentEl.clientHeight -
  ...
  private rotateIcon(): string {
    return this.rotateArrow && this.isPanelOpen
      ? this._iconOpened
      : this._iconClosed;
  }
}
